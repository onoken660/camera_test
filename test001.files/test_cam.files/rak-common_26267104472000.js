	...
	if (navigator.userAgent.indexOf('MSIE') > -1) {
		...
		if (navigator.userAgent.indexOf('Netscape6') > -1) {
			...
			if (navigator.userAgent.indexOf('Firefox') > -1) {
				...
				p_idx = navigator.userAgent.indexOf('Chrome') + 7;
			} else {
				p_idx = navigator.userAgent.indexOf('Mozilla') + 8;
			}
			var p_idx2 = p_idx;
			var p_versionNo;
			if (RakAgent.g_brwsub == RakAgent.CH) {
				p_versionNo = '0123456789';
			} else {
				p_versionNo = '.0123456789';
			}
			while (p_idx2 < navigator.userAgent.length
				&& p_versionNo.indexOf(navigator.userAgent.charAt(p_idx2)) > -1) {
				p_idx2++;
			}
			RakAgent.g_brwsver = navigator.userAgent.substring(p_idx, p_idx2);
		}
...
RakArrayUtil.move = function(x_array, x_fromIdx, x_toIdx) {
	if (RakUtil.isEmpty(x_array)) {
		return x_array;
	}
	var p_fromIdx = RakUtil.toInt(x_fromIdx, -1);
	var p_toIdx = RakUtil.toInt(x_toIdx, -1);
	if (p_fromIdx < 0 || p_toIdx < 0 
			|| p_fromIdx >= x_array.length 
			|| p_toIdx >= x_array.length
			|| x_fromIdx == x_toIdx) {
		return x_array;
	}
	var p_array = new Array();
	for (var i = 0; i < x_array.length; i++) {
		if (i == p_fromIdx) {
			...
			p_array.push(x_array[p_fromIdx]);
			p_array.push(x_array[i]);
		} else {
			p_array.push(x_array[i]);
		}
	}
...
RakArrayUtil.subtract = function(x_array1, x_array2) {
	if (RakUtil.isEmpty(x_array1) || RakUtil.isEmpty(x_array2)) {
		return x_array1;
	}
	var p_ret = new Array();
	for (var i = 0; i < x_array1.length; i++) {
		if (RakArrayUtil.contains(x_array2, x_array1[i])) {
			continue;
		}
		p_ret.push(x_array1[i]);
	}
	return p_ret;
};
var RakEventUtil = {};
...
RakElementUtil.addEventListener = function(x_id, x_type, x_function, x_capture, x_param) {
	if (!x_function) {
		return;
	}
	...
	if (p_elem.addEventListener) {
		...
		SeiAddEventListener(p_elem, x_type, x_function, x_param, x_capture);
	} else {
...
RakElementUtil.getTdWidth = function(x_td) {
	if (!this.g_tdwidthdiv) {
		var p_div = document.createElement('div');
		p_div.style.visibility = 'hidden';  
		p_div.style.overflow = 'auto';      
		p_div.style.height = '0px';         
		p_div.style.width = '0px';          
		var p_table = document.createElement('table');
		p_div.appendChild(p_table);
		p_table.style.borderCollapse = 'collapse';
		var p_tbody = document.createElement('tbody');
		p_table.appendChild(p_tbody);
		var p_tr = document.createElement('tr');
		p_tbody.appendChild(p_tr);
		var p_td = document.createElement('td');
		p_tr.appendChild(p_td);
		var p_child = document.createElement('div');
		p_td.appendChild(p_child);
		this.g_tdwidthdiv = p_div;
		this.g_tdwidthtable = p_table;
		this.g_tdwidthtd = p_td;
		this.g_tdwidthchild = p_child;
		document.body.appendChild(this.g_tdwidthdiv);
	}
	this.g_tdwidthtable.width = x_td.offsetWidth + 'px';
	this.g_tdwidthtd.style.padding = x_td.style.padding;
	this.g_tdwidthtd.style.paddingLeft = x_td.style.paddingLeft;
	this.g_tdwidthtd.style.paddingRight = x_td.style.paddingRight;
	this.g_tdwidthtd.style.border = x_td.style.border;
	this.g_tdwidthtd.style.borderLeft = x_td.style.borderLeft;
	this.g_tdwidthtd.style.borderRight = x_td.style.borderRight;
	return this.g_tdwidthchild.offsetWidth;
};
RakElementUtil.elem2px = function(x_elem) {
	if (!x_elem.childNodes) {
		return -1;
	}
	if (typeof(this.g_elem2px) == 'undefined') {
		var p_grandparent = document.createElement('div');
		p_grandparent.style.visibility = 'hidden';  
		p_grandparent.style.overflow = 'auto';
		p_grandparent.style.width = '0px';
		p_grandparent.style.height = '0px';
		var p_parent = document.createElement('div');
		p_grandparent.appendChild(p_parent);
		p_parent.style.height = '10px';
		p_parent.style.width = '4000px';
		var p_div = document.createElement('div');
		p_parent.appendChild(p_div);
		p_div.style.margin = 0;
		p_div.style.padding = 0;
		p_div.style.display = 'inline-block';
		document.body.appendChild(p_grandparent);
		this.g_elem2px = p_div;
	}
	this.g_elem2px.innerHTML = '';
	var p_childlen = x_elem.childNodes;
	for (var i = 0; i < p_childlen.length; i++){
		var p_clone = p_childlen[i].cloneNode(true);
		this.g_elem2px.appendChild(p_clone);
	}
	return Math.round(this.g_elem2px.offsetWidth * 1.05); 
};
var RakWindowUtil = {};
