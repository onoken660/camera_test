...
function SeiParseDate( x_value ){
  ...
  if (p_count == 1) {
    p_d = Number(p_val.substring(p_pos)); p_m = p_y;
    p_y = 2020;
  } else if (p_count == 2) {
...
function SeiParseDateWareki(x_value){
  ...
  for (var i in g_seiWarekiEra) {
    ...
    for (var j=1; j<p_era.length; j++) {
      ...
      var p_val2 = p_val;      p_val = p_val.substring(p_era[j].length)
      if (p_val.length == 6) {
        var p_y, p_m, p_d;
    p_y = Number(p_val.substring(0, 2));
    p_m = Number(p_val.substring(2, 4));
...
function SeiEditDateWareki(x_date) {
  ...
  if (p_date.getTime()>=600188400000) p_year = 'ïΩê¨' + (p_year - 1989 + 1);
